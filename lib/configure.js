...
const isMingw = win && (process.env.MSYSTEM || process.env.MINGW_PREFIX || process.env.CC === 'gcc' || process.env.CC === 'clang')
...
async function configure (gyp, argv) {
  ...
  // Debug: log the argv passed to configure
  log.verbose('configure', 'argv passed to configure: %j', argv)
  log.verbose('configure', 'gyp.opts: %j', gyp.opts)
  ...
  async function runGyp () {
    // Debug: log argv at the start of runGyp
    log.verbose('gyp', 'argv at start of runGyp: %j', argv)
    log.verbose('gyp', 'gyp.opts.format: %s', gyp.opts.format)
    
    // Check if format is specified via gyp.opts (from command line -f or --format)
    let formatSpecified = false
    
    if (gyp.opts.format) {
      formatSpecified = true
      // gyp expects -f format as two separate arguments
      argv.push('-f', gyp.opts.format)
      log.verbose('gyp', 'Using format from gyp.opts: %s', gyp.opts.format)
    } else {
      // Check if format is already in argv (shouldn't happen with our changes, but keep as fallback)
      const newArgv = []
      ...
      for (let i = 0; i < argv.length; i++) {
        ...
        if (arg === '-f' || arg === '--format') {
          ...
          log.verbose('gyp', 'Found format flag in argv: %s %s', arg, argv[i])
        } else if (arg.startsWith('--format=')) {
          formatSpecified = true
          const format = arg.substring('--format='.length)
          newArgv.push('-f', format)
          log.verbose('gyp', 'Found format flag in argv: %s -> -f %s', arg, format)
        } else if (arg.startsWith('-f=')) {
          formatSpecified = true
          const format = arg.substring('-f='.length)
          newArgv.push('-f', format)
          log.verbose('gyp', 'Found format flag in argv: %s -> -f %s', arg, format)
        } else {
          newArgv.push(arg)
      ...
      // Replace argv with processed arguments
      argv.length = 0
      argv.push(...newArgv)
    }
    
    log.verbose('gyp', 'formatSpecified: %s, argv after processing: %j', formatSpecified, argv)
    
    if (!formatSpecified) {
      if (win && !isMingw) {
        log.verbose('gyp', 'gyp format was not specified; forcing "msvs"')
        // force the 'msvs' target for Windows (non-MinGW)
        argv.push('-f', 'msvs')
      } else {
        log.verbose('gyp', 'gyp format was not specified; forcing "make"')
        // force the 'make' target for non-Windows and MinGW
        argv.push('-f', 'make')
      }
    }
