...
async function configure (gyp, argv) {
  ...
  async function runGyp () {
    ...
    // Check if format is already specified and handle --format=VALUE syntax
    ...
    for (let i = 0; i < argv.length; i++) {
      ...
      if (arg === '-f' || arg === '--format') {
        ...
        log.verbose('gyp', 'Found format flag: %s %s', arg, argv[i])
      } else if (arg.startsWith('--format=')) {
        formatSpecified = true
        const format = arg.substring('--format='.length)
        newArgv.push('-f', format)
        log.verbose('gyp', 'Found format flag: %s -> -f %s', arg, format)
